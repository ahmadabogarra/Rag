...
class DocumentAnalyzer:
    ...
    def _detect_fields(self, content: str, structure_type: str) -> List[Dict[str, str]]:
        """
        Detect unique fields in the document based on its structure type.
        ...
        if structure_type == 'structured':
            try:
                ...
                for header in headers:
                    sample = first_row.get(header, '')
                    add_field(
                        name=header,
                        description=f'حقل {header}',
                        sample=sample
                    )
                ...
                add_field(
                    name='content',
                    description='محتوى CSV',
                    sample=content[:100]
                )

        # تحليل المستندات شبه المنظمة (JSON, XML, Markdown)
            ...
            if content.startswith('{') or content.startswith('['):  # JSON
                try:
                    ...
                    add_field(
                        name='content',
                        description='محتوى JSON',
                        sample=content[:100]
                    )
            elif content.startswith('<') and '>' in content:  # XML
                try:
                    ...
                    add_field(
                        name='content',
                        description='محتوى XML',
                        sample=content[:100]
                    )
            elif re.search(r'^#+ ', content, re.MULTILINE):  # Markdown
                headers = re.findall(r'^(#+) (.+)$', content, re.MULTILINE)
                for level, header in headers:
                    add_field(
                        name=f'header_{level.count("#")}',
                        description=f'عنوان Markdown: {header}',
                        sample=header
                    )
                add_field(
                    name='content',
                    description='محتوى Markdown',
                    sample=content[:100]
                )
            else:
                add_field(
                    name='content',
                    description='محتوى شبه منظم',
                    sample=content[:100]
                )
        else:  # unstructured
            add_field(
                name='content',
                description='محتوى نصي كامل',
                sample=content[:100]
            )
        return fields
